...
using System;
using Binance.API.Trader.Utilitys;
    ...
    public class BinanceAPI
    {
        ...
        private void InitializeTraidingHistory()
        {
            foreach (Coin coin in coins)
            {
                ...
                {
                    var candlestick = (List<Candlestick>)binanceClient.GetCandleSticks(pair, TimeInterval.Minutes_1, null, null, 50).Result;
                    foreach (Candlestick stick in candlestick)
                    {
                        if (coin.Traiding)
                            coin.AddPrice(pair, stick.Close, stick.Volume);
                    }
                }
            }
        ...
        private static void MainLoop(object source, System.Timers.ElapsedEventArgs e)
        {
            foreach(Coin coin in coins)
            {
                ...
                foreach (string pair in TraidingPairList)
                {
                    var candlestick = (List<Candlestick>)binanceClient.GetCandleSticks(pair, TimeInterval.Minutes_1, null, null, 1).Result;
                    foreach (Candlestick stick in candlestick)
                    {
                        //Receive Data
